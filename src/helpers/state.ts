...
export type GenericListActionType = 'LIST_FETCH_STARTED' | 'LIST_FETCH_SUCCESS' | 'LIST_FETCH_ERROR' | 'LIST_REFRESH';
...
export const doGenericFetch =
  <Dto extends GridValidRowModel, DispatchType extends React.Dispatch<GenericAction<GenericListActionType>>>(
    url: string,
    extraParams?: Record<string, any>
  ) =>
  (startIndex = 0, count = 50, filter: string | null = null, orderBy: string | null = null, order: OrderDirection = 'asc') =>
  async (dispatch: DispatchType): Promise<void> => {
    try {
      dispatch({ type: 'LIST_FETCH_STARTED' });

      const response = await axios.get(url, {
        params: { startIndex, count, filter, orderBy, order, ...extraParams }
      });

      // === НОРМАЛИЗАЦИЯ ОТВЕТА ===
      const raw = response.data as any;

      // rows: предпочитаем .rows, но поддерживаем .items
      const rows: Dto[] = (raw?.rows ?? raw?.items ?? []) as Dto[];

      // totalCount: поддерживаем .totalCount, .total, .count
      ...
      // startIndex/count: не обязательно есть на бэке
      const normalized = {
        rows,
        totalCount,
        startIndex: typeof raw?.startIndex === 'number' ? raw.startIndex : startIndex,
      ...
      dispatch({
        ...
        payload: (error as any)?.response?.data?.message ?? 'Fetch error'
      });
    }
  };

export const doGenericReferenceLists =
  <DispatchType extends React.Dispatch<GenericAction<GenericActionType>>>(urls: Record<string, string>) =>
  () =>
  async (dispatch: DispatchType): Promise<void> => {
    try {
      dispatch({
...
export const doGenericFind =
  <DispatchType extends React.Dispatch<GenericAction<GenericActionType>>>(url: string) =>
  (id: number) =>
  async (dispatch: DispatchType): Promise<void> => {
    try {
      dispatch({
...
export const doGenericCreate =
  <Dto extends GridValidRowModel, DispatchType extends React.Dispatch<GenericAction<GenericActionType>>>(url: string, idField: keyof Dto) =>
  (values: Dto, onSuccess: () => void, onError: (message: string) => void) =>
  async (dispatch: DispatchType): Promise<void> => {
    try {
      dispatch({
...
export const doGenericUpdate =
  <Dto extends GridValidRowModel, DispatchType extends React.Dispatch<GenericAction<GenericActionType>>>(url: string) =>
  (id: number, values: Dto, onSuccess: () => void, onError: (message: string) => void) =>
  async (dispatch: DispatchType): Promise<void> => {
    try {
      dispatch({
...
export const genericActions = <Dto extends GridValidRowModel, DispatchType extends React.Dispatch<GenericAction<GenericActionType>>>(
  url: string,
  idField: keyof Dto
) => ({
